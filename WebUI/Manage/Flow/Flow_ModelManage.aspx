<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Flow_ModelManage.aspx.cs" Inherits="WC.WebUI.Manage.Flow.Flow_ModelManage" %>

<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">

<html xmlns="http://www.w3.org/1999/xhtml" >
<head id="Head1" runat="server">
    <meta http-equiv="X-UA-Compatible" content="IE=EmulateIE8"/><title>编辑工作流模型</title>
...
<script type="text/javascript">
	    function ShowIframe()
		{
            ...
            pop.setContent("title","工作流模型 - 审批人员选择");
            pop.build();
            pop.show();
        }             
        ...
        function ShowIframe_dep()
		{
            ...
            pop.setContent("title","工作流模型 - 审批职能部门选择");
            pop.build();
            pop.show();
        } 
        ...
        function ShowIframe_deps()
		{
            ...
            pop.setContent("title","工作流模型 - 职能部门选择");
            pop.build();
            pop.show();
        }         
...
<div id="PanelConfig">           
 ...
 <table class="table subsubmenu">
  <thead>
	<tr>
	  <td>  
	  <a href="Flow_ModelManage.aspx">新增流程模型</a>
	  </td>
...
<tr>
	<th style="width:155px;"><strong>* 工作流模型名称</strong>&nbsp;<a href="#" class="help">[?]</a></th>
	<td><span class="note">请输入工作流模型名称 如：请假申请流程</span>
	<input runat=server name="Flow_Name" type="text" dataType="Require" msg="工作流模型名称不能为空" id="Flow_Name" style="width:504px;" />
	</td>
</tr>
<tr>
	<th style="width:155px; font-weight:bold;">流程模型分类&nbsp;<a href="#" class="help">[?]</a></th>
	<td><span class="note">请选择模型所属的类别 </span>
	<asp:DropDownList runat=server ID=Model_Type style="width:515px;"></asp:DropDownList>
	</td>
</tr>
<tr runat=server id=trv >
	<th style="width:155px; font-weight:bold;">模板表单&nbsp;<a href="#" class="help">[?]</a></th>
	<td><span class="note">流程申请时 填写的模板文件表单 </span>
...
<tr>
	...
	<td><span class="note">该流程模型的 使用范围. (流程申请人的 所属部门)</span>
    &nbsp; <span style="color:#006600; font-weight:bold;">请在下方选择 该流程的使用范围。所有部门公用 请留空不填</span>
	</td>
</tr>
...
<tr>
	<th style="width:155px;"><strong>工作流模型名称</strong>&nbsp;</th>
	<td>&nbsp;
...
<tr>
	...
	<td><span class="note">请选择 审批人员：点击右侧 [审批人]或[审批部门] 按钮 </span>
        <div>
&nbsp;&nbsp;按人员选择：<input type=radio id=step_type0 name='step_type' value=0 checked=true  onclick='javascript:if(this.value==0){document.all.show2.style.display="none";document.all.show1.style.display=""}else{document.all.show1.style.display="none";document.all.show2.style.display=""}' /> &nbsp;&nbsp;
按部门选择：<input type=radio id=step_type1 name='step_type' value=1 onclick='javascript:if(this.value==0){document.all.show2.style.display="none";document.all.show1.style.display=""}else{document.all.show1.style.display="none";document.all.show2.style.display=""}' />
    </div>
	
	<div id=show1 style="margin-top:6px;display:'';">
...
<tr>
	...
	<asp:Panel runat=server ID=display1 Visible=true>
    &nbsp;&nbsp;是否编辑勾选附件 <input runat=server id="IsUserFile" checked=checked type="checkbox" name="IsUserFile" /> &nbsp;
	(说明：<strong>该步骤的审批者 </strong>是否可以 编辑勾选 该流程已上传审核附件) <br></asp:Panel>
	
	&nbsp;&nbsp;是否强制完成 <input runat=server id="RightToFinish" type="checkbox" name="RightToFinish" /> &nbsp;
	(说明：<strong>该步骤的审批者 </strong>是否可以 强行完成整个流程) 
...
<div style="text-align:center;">
	...
	<asp:Button runat=server id=finish1 OnClick=Finish_Btn CssClass="buttoms" Text=' 完成设计 ' OnClientClick="javascript:return confirm('确实要完成整个流程设计吗？')" />&nbsp;&nbsp; 
	<asp:Button runat=server id=cancer1 OnClick=Cancer_Btn CssClass="buttoms" Text=' 取消设计 ' OnClientClick="javascript:return confirm('取消设计将删除整个流程,确定吗？')" />&nbsp;&nbsp; 
</div>    
