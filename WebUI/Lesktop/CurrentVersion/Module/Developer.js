...
Core.Link("StyleSheet", Core.GetUrl("Module/Developer/StyleSheet.css"), "text/css");
...
'{PAD}{TAB}{{\r\n'+
'{PAD}{TAB}{TAB}m_Task=Taskbar.AddTask(This,Core.Utility.IsNull(config.Title.InnerHTML,""));\r\n'+
...
function init(completeCallback, errorCallback)
{
	function LoadModulesComplete()
	{
		...
		Window = Core.GetModule("Window.js").Window;
...
function RedirectCssUrl(dir, css)
{
	return css.replace(
		...
		function(u, url)
		{
			return String.format("url(\"{2}/{0}?UID={1}\")", url, Core.GenerateUniqueId(), dir);
		}
	);
}
...
{
	return Core.GetBrowser()=="IE"?"cur":"gif";
}
...
function GetOffset(evt, obj)
{
	...
	{
		var client = Core.Utility.GetClientCoord(obj);
...
function Post(callback, errorCallback, url, data, contentType)
{
	...
	if(request)
	{
		...
		request.setRequestHeader('Content-Type', Core.Utility.IsNull(contentType, 'application/x-www-form-urlencoded'))
			...
			if(request.readyState==4)
			{
				try
				{
					...
					case 200:
						...
						break;
					default:
						if(errorCallback) errorCallback(new Core.Exception("Server Error",request.statusText));
					}
				}
				finally
				{
...
function CodeBuilder(application) //CodeBuilder
{
	...
	'<br/>\r\n'+
	'<pre class="sh_javascript">\r\n'+
	'{PAD}Core.Link("StyleSheet", Core.GetUrl("{CSS}"), "text/css");\r\n'+
	'{PAD}\r\n'+
	...
	'{PAD}{TAB}{{\r\n'+
	'{PAD}{TAB}{TAB}Controls = Core.GetModule("Controls.js");\r\n'+
	'{PAD}\r\n'+
	'{PAD}{TAB}{TAB}Control = Core.GetModule("Controls.js").Control;\r\n'+
	'{PAD}\r\n'+
	'{PAD}{TAB}{TAB}_init(completeCallback, errorCallback);\r\n'+
	'{PAD}{TAB}}\r\n'+
	'{PAD}\r\n'+
	'{PAD}{TAB}Core.LoadModules(\r\n'+
	...
	'{PAD}{TAB}catch (ex)\r\n'+
	'{PAD}{TAB}{{\r\n'+
	'{PAD}{TAB}{TAB}errorCallback(new Core.Exception(ex.mame, ex.message));\r\n'+
	'{PAD}{TAB}}\r\n'+
	'{PAD}}\r\n'+
	'</pre>\r\n'+
	'{PAD}\r\n'+
	...
	{
		this.InitCode = Core.Utility.IsNull(data.InitCode, Default.InitCode);
		this.DisposeCode = Core.Utility.IsNull(data.DisposeCode, Default.DisposeCode);
		this.ModuleGlobalCode = Core.Utility.IsNull(data.ModuleGlobalCode, Default.ModuleGlobalCode);
...
function PlaceHolderBase(config)
{
	...
	This.Count = Core.Utility.IsNull(config.Count, 0);
	This.MaxIndex = 10;
	This.OnChanged = new Core.Delegate();
	This.OnFocus = new Core.Delegate();
	This.OnPropertyChanged = new Core.Delegate();

	This.HasSubControls = false;

	This.OnPHDblClick = new Core.Delegate();
...
function PlaceHolder(config)
{
	...
	var m_BorderWidth = Core.Utility.IsNull(config.ResizeBorderWidth,4);
...
function FormPH(config)
{
	...
	This.MemberCode = Core.Utility.IsNull(config.MemberCode, "");
	This.CustomConfigCode = Core.Utility.IsNull(config.CustomConfigCode, "");
	
	var m_Css = Core.Utility.IsNull(config._Css, "window");
	var m_HasMinBtn = Core.Utility.IsNull(config.HasMinButton, true);
	var m_HasMaxBtn = Core.Utility.IsNull(config.HasMaxButton, true);
	var m_Resizable = Core.Utility.IsNull(config.Resizable, true);
	...
	This.RenderCodeHtml = function(builder, paddingStr, paddingEM)
	{
		...
		builder.push(
			Format(
				...
				{
					NAME: This.GetName(),
					CONFIG: Core.Utility.RenderJson(fconfig),
					CTRLS: ctrlHtml.join("\r\n")
				}
			)
		);
...
function CustomControlDesignPH(config)
{
	...
	var BaseName=Core.Utility.IsNull(config.BaseName,"Control");
	
	var CustomConfig=Core.Utility.IsNull(config.CustomConfig,"");
	var GlobalCode=Core.Utility.IsNull(config.GlobalCode,"");
	...
	This.RenderCode = function(builder, paddingStr, paddingEM)
	{
		...
		builder.push(Format(
			FormCodeFormat, paddingStr, paddingEM,
			{
				NAME: This.GetName(), CONFIG: Core.Utility.RenderJson(fconfig), CTRLS: subs.join("\r\n"),
				WIDTH: This.GetWidth(),
				HEIGHT: This.GetHeight(),
...
function FramePH(config)
{
	...
	This.OnResizedCode = Core.Utility.IsNull(config.OnResizedCode, "");
	This.InitCode = Core.Utility.IsNull(config.InitCode, "");
	This.CustomConfigCode =  Core.Utility.IsNull(config.CustomConfigCode, "");
	var m_Css = Core.Utility.IsNull(config._Css, "control");
	...
	This.RenderCodeHtml = function(builder, paddingStr, paddingEM)
	{
		...
		builder.push(
			Format(
				...
				paddingStr, paddingEM,
				{ NAME: This.GetName(), FULLNAME: This.GetFullName(), CONFIG: Core.Utility.RenderJson(fconfig) }
			)
		);
...
function CustomControlObjectPH(config)
{
	...
	This.ConfigCode = Core.Utility.IsNull(config.ConfigCode, "");
	This.InitCode = Core.Utility.IsNull(config.InitCode, "");
	var m_Css = Core.Utility.IsNull(config._Css, "control");
	var m_ClassName = Core.Utility.IsNull(config.ClassName, "Control");
	...
	This.RenderCodeHtml = function(builder, paddingStr, paddingEM)
	{
		...
		builder.push(
			Format(
				ControlCodeHtmlFormat,
				paddingStr, paddingEM,
				{ NAME: This.GetName(), FULLNAME: This.GetFullName(), CONFIG: Core.Utility.RenderJson(fconfig),CLASS:m_ClassName}
			)
		);
...
function ButtonPH(config)
{
	...
	This.OnClickCode = Core.Utility.IsNull(config.OnClickCode, "");
	This.InitCode = Core.Utility.IsNull(config.InitCode, "");

	var m_Css = Core.Utility.IsNull(config._Css, "button");
...
function LabelPH(config)
{
	...
	var m_Css = Core.Utility.IsNull(config._Css, "label");
...
function TextBoxPH(config)
{
	...
	var m_Css = Core.Utility.IsNull(config._Css, "textbox");
...
function TextAreaPH(config)
{
	...
	var m_Css = Core.Utility.IsNull(config._Css, "textarea");
...
function CheckBoxPH(config)
{
	...
	var m_Css = Core.Utility.IsNull(config._Css, "checkbox");
...
function RadioButtonPH(config)
{
	...
	var m_Css = Core.Utility.IsNull(config._Css, "radio");
	var m_Group=Core.Utility.IsNull(config.Group,"");
...
function ListBoxPH(config)
{
	...
	var m_Css = Core.Utility.IsNull(config._Css, "listbox");
...
function DropDownListPH(config)
{
	...
	var m_Css = Core.Utility.IsNull(config._Css, "dropdownlist");
...
function ListViewPH(config)
{
	...
	This.InitCode = Core.Utility.IsNull(config.InitCode, String.format(DefaultCode.InitCode,This.GetName()));
	This.ListItem_Member=Core.Utility.IsNull(config.ListItem_Member,String.format(DefaultCode.ListItem_Member));
	This.ListItem_GetText=Core.Utility.IsNull(config.ListItem_GetText,String.format(DefaultCode.ListItem_GetText));

	var m_Css = Core.Utility.IsNull(config._Css, "listview");
	var m_BorderWidth=Core.Utility.IsNull(config._BorderWidth,1);
	var m_Columns=Core.Utility.IsNull(
		config._Columns,
		[
	...
	This.RenderCodeHtml = function(builder, paddingStr, paddingEM)
	{
		...
		builder.push(
			Format(
				CodeHtmlFormat,
				paddingStr, paddingEM,
				{ NAME: This.GetName(), FULLNAME: This.GetFullName(), CONFIG: Core.Utility.RenderJson(fconfig), COLUMNS:Core.Utility.RenderJson(m_Columns)}
			)
		);
	}
	...
	This.RenderCode = function(builder, paddingStr, paddingCount)
	{
		...
		builder.push(Format(
			CodeFormat, paddingStr, paddingCount,
			{
				...
				COLUMNS:Core.Utility.RenderJson(m_Columns)
			}
		));
	}
...
function TreeViewPH(config)
{
	...
	This.OnClickCode = Core.Utility.IsNull(config.OnClickCode, "");
	This.InitCode = Core.Utility.IsNull(config.InitCode, String.format("{0}.Refresh(function(){});",This.GetName()));
	This.DSMemberCode=Core.Utility.IsNull(config.DSMemberCode,"");
	This.DSGetSubNodesCode=Core.Utility.IsNull(config.DSGetSubNodesCode,"callback([]);");

	var m_Css = Core.Utility.IsNull(config._Css, "treeview");
	var m_BorderWidth=Core.Utility.IsNull(config._BorderWidth,1);
...
function RichEditorPH(config)
{
	...
	var m_Css = Core.Utility.IsNull(config._Css, "richEditor");
	var m_BorderWidth=Core.Utility.IsNull(config._BorderWidth,1);
...
function SimpleTabControlPH(config)
{
	function TabsSettingForm()
	{
        ...
        m_BtnAdd.OnClick.Attach(
            function(btn)
            {
				appendItem(new m_ListTabs_ListItem("新标签",100,Core.GenerateUniqueId()));
            }
        );
	...
	var m_DefaultTabs = [
		{ Text: "标签1", Width: 80, IsSelected: true, ID: Core.GenerateUniqueId() },
		{ Text: "标签2", Width: 80, IsSelected: false, ID: Core.GenerateUniqueId() }
	];
	...
	else
	{
		...
		{
			m_Tabs[i].ID=Core.GenerateUniqueId();
	...
	This.OnSelectedTabCode = Core.Utility.IsNull(config.OnSelectedTabCode, "");
	This.InitCode = Core.Utility.IsNull(config.InitCode, "");
	var m_Css = Core.Utility.IsNull(config._Css, "simple_tab");
	var m_BorderWidth=Core.Utility.IsNull(config._Broderwidth,1);
...
function ToolbarPH(config)
{
	...
	This.OnCommandCode = Core.Utility.IsNull(config.OnCommandCode, "");
	This.InitCode = Core.Utility.IsNull(config.InitCode, "");
	var m_Css = Core.Utility.IsNull(config._Css, "toolbar");
	var m_Items=Core.Utility.IsNull(config._Items,[{ Css: "", Text: "Button", Command: "Cmd" }]);
...
function CssEditor(config)
{
	...
	This.Load = function(config)
	{
		...
		m_Editor.SetText(Core.Utility.IsNull(config.CssCode, m_DefalutCode));
	}
...
function CodeEditor(config)
{
	...
	var m_OnInit=Core.Utility.IsNull(config.OnInit,null);
...
function OpenSolutionForm()
{
    ...
    {
        m_Task = Taskbar.AddTask(This, Core.Utility.IsNull(config.Title.InnerHTML, ""));
    ...
    var label2 = new Controls.Label(
		{
			...
			"AnchorStyle": Controls.AnchorStyle.Left | Controls.AnchorStyle.Right | Controls.AnchorStyle.Top,
			"Parent": This,
			"Text": String.format("1.将以下代码保存到网站服务器上（请使用UTF-8编码）;<br/>2.将Css保存到\"{0}\";<br/>3.调用Core.Exec(function(){},alert,'代码路径.js')运行。", css),
			"Css": "label"
		}
    );
    ...
    var m_LabelInfo = new Controls.Label(
		{
			...
			Text: Core.Utility.IsNull(config.Tip, ""),
			Parent: This
		}
    );
    ...
    var m_EditPrompt = new Controls.TextBox(
		{
			...
			AnchorStyle: Controls.AnchorStyle.Left | Controls.AnchorStyle.Right | Controls.AnchorStyle.Bottom,
			Text: Core.Utility.IsNull(config.DefaultValue, ""),
			Parent: This
		}
    );
...
function Instance()
{
	...
	{
		return Core.Path.GetFileNameNoExtention(m_SolutionFile);
	}
	...
	{
		return Core.Path.GetDirectoryName(m_SolutionFile) + "/Bin";
	}
	...
	function MainForm()
	{
			...
			dialog.ShowDialog(
				...
				function()
				{
					...
					var postData = String.format(
						...
						"</Data>",
						dialog.GetPath(),
						Core.Utility.TransferCharForXML(Core.Utility.RenderJson(data))
					);
					...
					try
					{
						Post(
							function(xml, text)
							{
								...
								var ret = Core.Utility.ParseJson(text);
								if(ret.Result == "OK")
								{
					...
					function(xml, text)
					{
						...
						if (ret.Result == "OK")
						{
							...
							try
							{
								...
								var data = Core.Utility.ParseJson(ret.Data.Solution);
		...
		function Save(completedCallback, errorCallback)
		{
			function render()
			{
				...
				var content = Core.Utility.RenderJson(data);
				return content;
			}
			...
			var postData = String.format(
				...
				"</Data>",
				m_SolutionFile,
				Core.Utility.TransferCharForXML(content),
				Core.Utility.TransferCharForXML(code),
				Core.Utility.TransferCharForXML("")
			);
