...
function Control(config) {
    ...
    var _borderWidth = Core.Utility.IsNull(config.BorderWidth, 0);
    This.GetBorderWidth = function () { return _borderWidth; }

    var _padding = Core.Utility.IsNull(config.Padding, 0);
    This.GetPadding = function () { return _padding; }

    //框架
    var _dom = Core.Utility.IsNull(config.Dom, document.createElement("DIV"));
    ...
    This.OnResized = new Core.Delegate();
    ...
    //AnchorStyle
    var _anchorStyle = Core.Utility.IsNull(config.AnchorStyle, AnchorStyle.Left | AnchorStyle.Top);
    ...
    This.OnDispose = new Core.Delegate();
    ...
    //初始化位置和大小
    This.Move(Core.Utility.IsNull(config.Left, 0), Core.Utility.IsNull(config.Top, 0));
    This.Resize(Core.Utility.IsNull(config.Width, 200), Core.Utility.IsNull(config.Height, 150));
    This.SetCss(Core.Utility.IsNull(config.Css, "control"));
    This.SetParent(Core.Utility.IsNull(config.Parent, null));
    This.SetVisible(Core.Utility.IsNull(config.Visible, true));
}
...
function Label(config) {
    ...
    //by fj 2011-7-20增加点击事件
    This.OnClick = new Core.Delegate();
    ...
    //by fj 2011-7-20增加点击事件
    This.GetDom().onclick = function (evt) {
    ...
    This.SetText = function (value) {
        This.GetDom().innerHTML = Core.Utility.ReplaceHtml(value);
    }
    ...
    This.SetTitle = function (title) {
        This.GetDom().title = title;
        This.GetDom().style.cursor = "pointer";
    }
    //end
    This.SetText(Core.Utility.IsNull(config.Text, ""));
    This.SetCss(Core.Utility.IsNull(config.Css, "label"));
}
...
function Button(config) {
	...
	'</tr>' +
	'</table>'
    _dom.tabIndex = Core.Utility.IsNull(config.TabIndex, -1);
    ...
    This.SetText = function (value) {
        This.GetDom().firstChild.rows[0].cells[1].innerHTML = Core.Utility.ReplaceHtml(value);
        Base.SetText(value);
    }
    ...
    Core.Utility.DisableSelect(_dom, true);

    _dom = null;

    This.SetText(Core.Utility.IsNull(config.Text, ""));
    This.SetCss(Core.Utility.IsNull(config.Css, "button"));
...
function TextBox(config) {
    ...
    This.SetText(Core.Utility.IsNull(config.Text, ""));
    This.Resize(This.GetWidth(), This.GetHeight());
    This.SetCss(Core.Utility.IsNull(config.Css, "textbox"));
}
...
function TextArea(config) {
    ...
    This.SetText(Core.Utility.IsNull(config.Text, ""));
    This.Resize(This.GetWidth(), This.GetHeight());
    This.SetCss(Core.Utility.IsNull(config.Css, "textarea"));
}
...
function CheckBox(config) {
    ...
    This.SetText = function (value) {
        _textdom.innerHTML = Core.Utility.ReplaceHtml(value);
    }
    ...
    //by增加设置Checkbox的值 wenjl 2011-5-3
    ...
    This.SetText(Core.Utility.IsNull(config.Text, ""));
    This.Resize(This.GetWidth(), This.GetHeight());
    This.SetCss(Core.Utility.IsNull(config.Css, "checkbox"));
}
...
function RadioButton(config) {
    ...
    This.SetText(Core.Utility.IsNull(config.Text, ""));
    This.Resize(This.GetWidth(), This.GetHeight());
    This.SetCss(Core.Utility.IsNull(config.Css, "radio"));
}
...
function DropDownList(config) {
    ...
    This.SetText(Core.Utility.IsNull(config.Text, ""));
    This.Resize(This.GetWidth(), This.GetHeight());
    This.SetCss(Core.Utility.IsNull(config.Css, "select"));
}
...
function ListBox(config) {
    ...
    This.GetDropDownListDom().size = Core.Utility.IsNull(config.Size, 4);
...
function TreeView(config) {
    ...
    var nodeHeight = Core.Utility.IsNull(config.NodeHeight, 20);
    var hasImage = Core.Utility.IsNull(config.HasImage, false);

    var dom = This.GetDom();
    dom.innerHTML = "<div style='overflow:visible;'></div>";

    This.OnDblClick = new Core.Delegate();
    This.OnClick = new Core.Delegate();
    This.OnExpand = new Core.Delegate();
    This.OnCollapse = new Core.Delegate();
    This.OnBeginRequest = new Core.Delegate();
    This.OnEndRequest = new Core.Delegate();
    //by fj 2011-4-22 增加外部事件
    This.OnMouseover = new Core.Delegate();
    This.OnMouseout = new Core.Delegate();
    /*
    获取子节点
    name:节点名称
				...
				function (subNodeData, error) {
				    if (subNodeData != null) {
				        ...
				        for (var i in subNodeData) {
				            ...
				            //增加显示Title by fj 2011-4-18   2011-6-24增加checkbox
				            var node = new TreeNode(This, null, nd.Name, nd.Text, nd.ImageCss, nd.Tag, dom.firstChild, dataSource, nodeHeight, i == subNodeData.length - 1, nd.HasChildren, nd.ImageSrc, nd.Tootip, nd.HasCheckBox);
				            rootNodes.push(node);
    ...
    */
    This.Find = function (callback, path) {
        ...
        if (pathNodes.length > 1 && pathNodes[0] == "") {
            ...
            callback(null, new Core.Exception("Error", "路径格式不正确!"));
        }
    }
    ...
    */
    ...
    //begin by fj 2011-6-24增加checkbox 
...
//增加显示Title by fj 2011-4-18   2011-6-24增加checkbox
function TreeNode(treeView, parentNode, name, text, imgSrc, tag, container, dataSource, height, isLast, hasChildren, imagePath, Tootip, hasCheckBox) {
    if (hasChildren == undefined || hasChildren == null) hasChildren = true;
    //if (Title == undefined || Title == null) Title = "";
    //by fj 2011-6-24增加checkbox
    ...
    This.SetText = function (val) {
        text = val;
        textDiv.innerHTML = Core.Utility.ReplaceHtml(val);
    }
            ...
            dataSource.GetSubNodes(
				function (subNodeData, error) {
				    if (subNodeData != null) {
				        ...
				        for (var i in subNodeData) {
				            var nd = subNodeData[i];
				            //增加显示Title by fj 2011-4-18
				            var node = new TreeNode(treeView, This, nd.Name, nd.Text, nd.ImageCss, nd.Tag, subNodesContainer, dataSource, height, i == subNodeData.length - 1, nd.HasChildren, nd.ImageSrc, nd.Tootip, nd.HasCheckBox);
				            subNodes.push(node);
				            nodeIndex[nd.Name] = node;
				        }
				        if (This.IsExpand()) {
    ...
    */
    ...
    buttonDiv.onclick = function () {
        if (This.IsExpand()) This.Collapse(); else This.Expand(function (result, error) { if (!result) Core.Utility.ShowError(error) });
    }
    ...
    var treeNodeName = "node" + Core.Utility.ReplaceHtml(name.toUpperCase());
    if (parentNode != null) {
        treeNodeName = "node" + Core.Utility.ReplaceHtml(parentNode.GetText().toUpperCase()) + Core.Utility.ReplaceHtml(name.toUpperCase());
    }
    if (parentNode != null && parentNode.GetTag() != null) {
        treeNodeName = "node" + Core.Utility.ReplaceHtml(parentNode.GetTag().Name.toUpperCase()) + Core.Utility.ReplaceHtml(name.toUpperCase());
    }
    textCell.innerHTML = String.format("<div class='nodeText' id={2} title={1} >{0}</div>", Core.Utility.ReplaceHtml(text), Core.Utility.ReplaceHtml(Tootip.Content), treeNodeName);
    //end
    ...
    //begin 增加事件处理 by fj 2011-4-22
    //鼠标移开
    imgCell.firstChild.onmouseout = function () {
        treeView.OnMouseout.Call(This);
    }
    ...
    //鼠标移上去
    imgCell.firstChild.onmouseover = function () {
        treeView.OnMouseover.Call(This);
    }
...
function ListView(config) {
    ...
    This.OnItemDblClick = new Core.Delegate();

    var _columns = [];

    var _enableMultiSelect = Core.Utility.IsNull(config.EnableMultiSelect, false);
    var _viewMode = Core.Utility.IsNull(config.ViewMode, "List");
    var _list = null;
        ...
        case "List":
            {
                _list = {
                    Config: {
                        HeaderHeight: Core.Utility.IsNull(config.ListMode.HeaderHeight, 28),
                        RowHeight: Core.Utility.IsNull(config.ListMode.RowHeight, 24)
                    }
                };
    ...
    This.Reset(Core.Utility.IsNull(config.Columns, []));
    ...
    function ListViewItem(values, css) {
        ...
        if (_viewMode == "List") {
            ...
            for (var i in _columns) {
                ...
                colDom.innerHTML = Core.Utility.ReplaceHtml(val);
        ...
        dom.onmousedown = function (evt) {
            ...
            else if (Core.Utility.GetButton(evt) == "Left") {
                if (_enableMultiSelect) {
                    if (evt.ctrlKey) {
                        if (evt.shiftKey) {
                            if (_latestItem != null) {
    ...
    function ListHeader() {
        ...
        This.OnColumnClick = new Core.Delegate();
        ...
        This.Create = function () {
            ...
            for (var i in _columns) {
                ...
                colDom.style.width = col.GetWidth() + 'px';
                width += col.GetWidth();
                colDom.className = Core.Utility.IsNull(col.GetCss(), "td_header_default");
                colDom.innerHTML = String.format("<span>{0}</span>", Core.Utility.ReplaceHtml(col.GetText()));
                (function (index) {
    ...
    function ItemPanel() {
        ...
        This.OnScroll = new Core.Delegate();
...
function Frame(config) {
    ...
    This.OnBeforeLoad = new Core.Delegate();
    This.OnAfterLoad = new Core.Delegate();
    ...
    This.SetCss(Core.Utility.IsNull(config.Css, "frame"));
    This.Resize(This.GetWidth(), This.GetHeight());
...
function HtmlListBox(css, width) {
    ...
    this.AddItem = function (text, value) {
        ...
        item_dom.innerHTML = Core.Utility.ReplaceHtml(text);
...
function HtmlDropDownList(css, width, height) {
    ...
    var dom = document.createElement("DIV");
    dom.className = Core.Utility.IsNull(css, "html_dropdownlist");
    ...
    this.OnChanged = new Core.Delegate();
    ...
    this.SetText = function (text) {
        ...
        for (var i in items) {
            if (items[i].Text == text) {
                dom.firstChild.firstChild.innerHTML = Core.Utility.ReplaceHtml(text);
                This.OnChanged.Call();
            }
        }
    }
    ...
    this.SetValue = function (value) {
        ...
        for (var i in items) {
            if (items[i].Value == value) {
                dom.firstChild.firstChild.innerHTML = Core.Utility.ReplaceHtml(items[i].Text);
                This.OnChanged.Call();
            }
        }
    }
    ...
    This.ShowList = function () {
        if (!m_IsListVisible) {
            var coord = Core.Utility.GetClientCoord(dom);
            ...
            Core.Utility.AttachEvent(document, "click", document_onclick);
        }
    }
    ...
    This.HideList = function () {
        if (m_IsListVisible) {
            ...
            Core.Utility.DetachEvent(document, "click", document_onclick);
        }
    }
...
function Toolbar(config) {
    ...
    This.CreateControl = function (item) {
        if (item.Type == undefined || item.Type == "Button") {
            ...
            if (item.Text == "") {
                ...
                Core.Utility.AttachButtonEvent(dom, "toolbar_btn", "toolbar_btn_hover", "toolbar_btn_hover");
            }
            else {
                dom.className = "toolbar_text_btn";
                Core.Utility.AttachButtonEvent(dom, "toolbar_text_btn", "toolbar_text_btn_hover", "toolbar_text_btn_hover");
            }
            dom.innerHTML = String.format("<div>{0}</div>", Core.Utility.ReplaceHtml(item.Text), Toolbar.PredefineHeight);
        ...
        else if (item.Type == "Image") {            //扩展了image类型2011-4-12 by fj 
            var dom = document.createElement("DIV");
            ...
            dom.innerHTML = String.format("<div title={0} style='width:0px;height:22px;'></div>", Core.Utility.ReplaceHtml(item.Text), Toolbar.PredefineHeight);
            dom.firstChild.className = item.Css;
    ...
    if (config.DisableSelect == undefined || config.DisableSelect) {
        Core.Utility.DisableSelect(This.GetDom(), true);
    }
    ...
    This.SetButtonText = function (index, text) {
        m_Btns[index].firstChild.innerHTML = Core.Utility.ReplaceHtml(text);
    }
...
function SimpleTabControl(config) {
    ...
    This.Reset = function (tabs) {
        ...
        for (var i in tabs) {
            var tabConf = tabs[i];
            var tab = new Tab(tabConf.Text, Core.Utility.IsNull(tabConf.Width, 0));
            m_Tabs.push(tab);
    ...
    This.OnSelectedTab = new Core.Delegate();
    ...
    This.Reset(Core.Utility.IsNull(config.Tabs, []));

    function Tab(text, width) {
        var tab = this;

        var dom = document.createElement("DIV");
        dom.innerHTML = Core.Utility.ReplaceHtml(text);
