...
function init(completeCallback, errorCallback) {
    Core.LoadModules(
		function() {
		    ...
		    AnchorStyle = Core.GetModule("Controls.js").AnchorStyle;
		    completeCallback();
		},
		errorCallback,
		["Controls.js"]
...
function Window(config) {
    ...
    var baseConfig = Core.Utility.Clone(config);
    ...
    This.OnHided = new Core.Delegate();
    This.onclosing = new Core.Delegate();
    ...
    if (config.Title != undefined) {
        ...
        if (config.Title.InnerHTML != undefined) _title.InnerHTML = Core.Utility.ReplaceHtml(config.Title.InnerHTML);
    }

    var m_ControlMode = Core.Utility.IsNull(config.ControlMode, false);

    var _id = Core.GenerateUniqueId();
    var dom = This.GetDom();
    dom.tabIndex = -1;
    ...
    var m_MinHeight = Core.Utility.IsNull(config.MinHeight, 300);
    var m_MinWidth = Core.Utility.IsNull(config.MinWidth, 400);
    ...
    var m_BkImage = new Controls.HtmlControlBkImage(
		Core.Utility.IsNull(config.BkImage, { Horiz: [6, 100, 6], Vertical: [24, 100, 6], Css: "window_bk" })
	);
    ...
    domTitle.childNodes[1].innerHTML = Core.Utility.ReplaceHtml(_title.InnerHTML);
    ...
    var _client = new Control(
		{
		    ...
		    BorderWidth: 0,
		    Parent: This,
		    Css: Core.Utility.IsNull(config.ClientCss, "")
		}
	);
    ...
    var _waiting = new Control(
		{
		    ...
		    BorderWidth: 0,
		    Parent: This,
		    Css: Core.Utility.IsNull(config.Waiting, "waiting")
		}
	);
    ...
    This.Resize(Core.Utility.IsNull(config.Width, 600), Core.Utility.IsNull(config.Height, 450));
