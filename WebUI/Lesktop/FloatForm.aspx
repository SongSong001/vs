...
<head runat="server">
	...
	function AddFriend()
	{
		Core.LoadModules(
			function()
			{
				Core.GetModule("Common.js").AddFriend(
					function(result, ex)
					{
						if(!result) Core.Utility.ShowError(ex);
						CurrentWindow.Close();
					},
					Data.Peer.Name
				);
	...
	function AddToGroup()
	{
		Core.LoadModules(
			function()
			{
				Core.GetModule("Common.js").AddToGroup(
					function(result, ex)
					{
						if(!result) Core.Utility.ShowError(ex);
						CurrentWindow.Close();
	...
	function CreateMessageText(json)
	{
		...
		else if(Data.Type == "DeleteFriendNotify")
		{
			...
			if(Core.Session.GetUserName().toUpperCase() == Data.User.Name.toUpperCase())
			{
				return String.format(
					"您已将 {0}({1}) 从好友中删除！",
	...
	function ShowMessage(json, type)
	{
        ...
        //设置弹出窗口显示时间 by fj 2011-4-20
		var showTime = "<%=floatWindowShowTime%>";
		if (showTime == "" || showTime == undefined)
		    showTime = 20000;
		setTimeout(function () { CurrentWindow.Close(); }, showTime);
	}
	</script>
</head>
<body>
